...
- **DCCore Chat, in the mIRC window script** - public chat with other operators in the channels you already share, over a NOTICE starting with a neutral `[ServersChat]` tag, sent from the operator's own client with no bot involved. Its own window, which says it is public; channels picked and listened on from its right-click menu; a per-nick flood limit; colours stripped; nothing ever sent automatically. See [ADMIN-CONSOLE.md](ADMIN-CONSOLE.md).
