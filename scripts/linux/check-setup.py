...
if os.name != "posix":
    warn("this check is written for Linux; on Windows use windows\\check-setup.py")
else:
    ok("running on Linux")
...
if rar:
    ...
    warn("no rar binary found on PATH - whole-album (!rar) packing will fail, "
...
if not masks:
    ...
    warn("ADMIN_HOSTMASKS is set but ADMIN_PASSWORD_HASH is empty - the console "
         "will refuse every connection until you run: python3 adminchat.py")
else:
